...
The **system prompt** is a special message that sets the model's behavior for the entire conversation. It gets elevated attention from the model — instructions in the system prompt are weighted more heavily than the same instructions placed in a user message.
